  ...
  userWords.forEach((word) => {
    ...
    if (datasetDemWord) {
      ...
      if ((demSentiment >= 0 && userSentiment >= 0) || (demSentiment < 0 && userSentiment < 0)) {
        points.dem += (featureWeightSharedWords * demSentiment * demSalience) / totalDemSentimentAndSalience;
      } else {
        points.rep += (featureWeightSharedWords * demSentiment * demSalience) / totalDemSentimentAndSalience;
      }
    }
    if (datasetRepWord) {
      ...
      if ((repSentiment >= 0 && userSentiment >= 0) || (repSentiment < 0 && userSentiment < 0)) {
        points.rep += (featureWeightSharedWords * repSentiment * repSalience) / totalRepSentimentAndSalience;
      } else {
        points.dem += (featureWeightSharedWords * repSentiment * repSalience) / totalRepSentimentAndSalience;
      }
    }
  });
  return points;
